import dash
import dash_bootstrap_components as dbc     # för enkel styling (Flask för att skapa webserver)
import os
from load_data import StockData
from dash import html, dcc  # dash core components
from dash.dependencies import Output, Input     #for callbacks så något händer när man trycker på knappar
...
def highest_lowest_value_update(json_df, ohlc): #funktionen körs när Input ovan ändras och outputen går in som Output
    dff = pd.read_json(json_df)
    highest_value = dff[ohlc].max()
    lowest_value = dff[ohlc].min()
    return highest_value, lowest_value      #return f"MAX: {highest_value}, lowest_value" om vill skiva ut MAX: värde på hemsidan
