...
df_dict = {symbol: stockdata_object.stock_dataframe(symbol) for symbol in symbol_dict} # dict av dataframes, använder bara keya

stock_options_dropdown = [{"label": name, "value": symbol} for symbol, name in symbol_dict.items()] # får lista av 4 dicts, name kommer synas
...
app.layout = html.Main(
    [
        ...
        dcc.RadioItems(id = "ohlc-radio", options = ohlc_options, value = "close"), #open high low close
...
def update_graph(stock, ohlc, time_index): #ordning på argumenten enligt app.callback ovan
    # tuple unpacks a list (dff är dataframe filtered)
